...
import { articleVisuals, parseArticleBody, programCatalog, type PublicArticle } from "@/lib/content";
...
export function SiteHeader() {
  ...
  const goTo = (target: string) => {
    ...
    if (target.startsWith("#")) {
      const scrollToTarget = () => document.querySelector(target)?.scrollIntoView({ behavior: "smooth", block: "start" });
      if (location !== "/") {
        setLocation("/");
        window.setTimeout(scrollToTarget, menuWasOpen ? 220 : 80);
      } else window.setTimeout(scrollToTarget, menuWasOpen ? 220 : 0);
      return;
    }
    setLocation(target);
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const message = deliveryStatus === "sent"
        ? `Your full PDF${planNames.length === 1 ? " is" : "s are"} attached, ${firstName}. Check this inbox now.`
...
export function EmailPopup() {
  ...
  const cartSuccess = deliveryStatus === "sent"
    ? "Your selected full PDFs are attached in your email. Check it now."
    : deliveryStatus === "limit_reached"
...
export function ArticleCard({ article, featured = false }: { article: PublicArticle; featured?: boolean }) { const cover = parseArticleBody(article.body).cover; return <Link href={`/articles/${article.slug}`} className={`article-card ${featured ? "featured" : ""}`}><div className={`article-visual ${cover ? "has-cover" : articleVisuals[article.slug] ?? "visual-swoop"}`}>{cover ? <div className="article-cover-canvas"><img className="article-guide-image" src={cover.src} alt={cover.alt} decoding="async" /></div> : <><span>{article.category}</span><Dumbbell aria-hidden="true" size={featured ? 58 : 38} /></>}</div><div className="article-card-copy"><p>{article.category}</p><h3>{article.title}</h3><span>{article.excerpt}</span><strong>Read guide <ArrowDownRight size={16} /></strong></div></Link>; }
...
export function ProgramCarousel() {
  ...
  return <div className="program-carousel"><div className="program-strip" ref={strip} role="region" aria-label="Scroll through program PDFs" tabIndex={0}>{programCatalog.map((program, index) => <article className={`program-pdf ${program.accent} ${activePlan === program.title ? "is-pressed" : ""}`} key={program.title} onPointerDown={() => setActivePlan(program.title)} onPointerUp={() => setActivePlan(null)} onPointerCancel={() => setActivePlan(null)} onPointerLeave={() => setActivePlan(null)}><div className="program-pdf-tablet"><div className="program-pdf-matte"><div className="program-pdf-cover"><img className="program-pdf-cover-image" src={program.cover} alt={`${program.title} PDF cover`} decoding="async" /></div></div></div><div className="program-pdf-copy"><p className="eyebrow">0{index + 1} / {programCatalog.length} · {program.tag}</p><h3>{program.title}</h3><p>{program.subtitle}</p><strong>{program.price}</strong><button className="black-button" type="button" onClick={() => addToCart(program)}>Add to cart <ShoppingCart size={16} /></button></div></article>)}</div><div className="carousel-controls"><span>SCROLL THE PDF. DO THE WORK.</span><button type="button" onClick={() => move(-1)} aria-label="Previous program"><ArrowLeft size={18} /></button><button type="button" onClick={() => move(1)} aria-label="Next program"><ArrowRight size={18} /></button></div></div>;
}
