...
export function ProgramCarousel({ onFreePlan }: { onFreePlan: () => void }) {
  ...
  const next = () => setActive((index) => (index + 1) % programs.length);
  const previous = () => setActive((index) => (index - 1 + programs.length) % programs.length);
  return <div className="program-carousel"><div className={`program-sheet ${current.accent}`}><div className="program-badge">{active + 1}<small>/ {programs.length}</small></div><div className="program-art"><span className="program-ring" /><Dumbbell size={86} /><span className="program-dots" /></div><div className="program-copy"><p className="eyebrow">{current.tag}</p><h3>{current.title}</h3><p>{current.copy}</p><strong>{current.price}</strong><button className="black-button" type="button" onClick={onFreePlan}>{active === 0 ? "Get the free plan" : "Join waitlist"}<ArrowRight size={16} /></button></div></div><div className="carousel-controls"><button type="button" onClick={previous} aria-label="Previous program"><ArrowLeft size={18} /></button><span>SWIPE THE PLAN. DO THE WORK.</span><button type="button" onClick={next} aria-label="Next program"><ArrowRight size={18} /></button></div></div>;
}
