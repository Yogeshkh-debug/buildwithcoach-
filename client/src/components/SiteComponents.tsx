...
export function ArticleCard({ article, featured = false }: { article: PublicArticle; featured?: boolean }) { const cover = parseArticleBody(article.body).cover; return <Link href={`/articles/${article.slug}`} className={`article-card ${featured ? "featured" : ""}`}><div className={`article-visual ${articleVisuals[article.slug] ?? "visual-swoop"}`}>{cover ? <img className="article-guide-image" src={cover.src} alt={cover.alt} decoding="async" /> : <><span>{article.category}</span><Dumbbell aria-hidden="true" size={featured ? 58 : 38} /></>}</div><div className="article-card-copy"><p>{article.category}</p><h3>{article.title}</h3><span>{article.excerpt}</span><strong>Read guide <ArrowDownRight size={16} /></strong></div></Link>; }
